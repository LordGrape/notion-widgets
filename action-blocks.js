...
function notionPayload(tasks){return parseList(tasks).filter(function(t){return t&&t.occurrenceId&&t.scheduledStart}).map(function(t){var duration=Math.max(1,Math.round((Date.parse(t.scheduledEnd||t.scheduledStart)-Date.parse(t.scheduledStart))/60000));return{notionPageId:t.notionPageId||undefined,occurrenceId:t.occurrenceId,scheduleId:t.scheduleId||undefined,action:t.text||'Scheduled action',category:cap(t.category)||'Personal',status:t.outcome==='skipped'?'Skipped':t.done?'Done':'Scheduled',scheduledStart:t.scheduledStart,scheduledEnd:t.scheduledEnd||null,plannedMinutes:duration,actualMinutes:t.actualMinutes||null,priority:cap(t.pri),source:t.source==='timetable'?'Schedule':'Manual',notes:[t.outcomeGoal?('Finish line: '+t.outcomeGoal):'',t.notes||''].filter(Boolean).join(' · ')}})}
