/* Action Blocks v1
   Deterministic bridge for timetable, to-do, SyncEngine, and Notion.
   No AI scheduling: only explicitly tracked schedule blocks become actions. */
(function(root){
'use strict';
/* ACTION BLOCKS EDITOR v2 */
/* SEAMLESS PLANNING SYSTEM v1 */
/* ADAPTIVE WEEKLY TARGETS v1 */
/* CLEAR SCHEDULE LANGUAGE v1
   Behaviour retained: Weekly to-do; Fresh task every scheduled day. */
...
function parseList(value){if(Array.isArray(value))return value;if(typeof value==='string')try{var p=JSON.parse(value);return Array.isArray(p)?p:[]}catch(e){}return[]}
function inferCategory(block){
 var text=((block&&block.name)||'')+' '+((block&&block.description)||'');
 if(/\b(LAW|POLS|ECON|HIST|PSYC|SOC|PHIL|MATH|BIOL|CHEM|PHYS)\s*[- ]?\d{2,4}[A-Z]?\b/i.test(text)||/^\s*\d{2,4}[A-Z]?\s*[:\-–|]/.test(text))return'class';
 if(/\b(gym|workout|strength|lift|lifting|run|running|ruck|rucking|cardio|zone\s*2|interval|sprint|soccer|mobility|recovery|swim|rowing|pilates)\b/i.test(text))return'training';
...
function normalizeOverride(value){
 ...
 if(value.start)value.start=String(value.start);if(value.end)value.end=String(value.end);if(Object.prototype.hasOwnProperty.call(value,'location'))value.location=String(value.location||'');if(Object.prototype.hasOwnProperty.call(value,'outcomeGoal'))value.outcomeGoal=String(value.outcomeGoal||'').trim();return value
}
...
function occurrenceEntries(block,when){
 ...
 (Array.isArray(block.days)?block.days:[]).filter(function(entry){return+entry.day===day}).forEach(function(entry){
  ...
  result.push({day:day,start:direct&&direct.start||entry.start,end:direct&&direct.end||entry.end,location:direct&&Object.prototype.hasOwnProperty.call(direct,'location')?direct.location:(entry.location||block.location||''),outcomeGoal:direct&&Object.prototype.hasOwnProperty.call(direct,'outcomeGoal')?direct.outcomeGoal:(block.outcomeGoal||''),sourceDate:key})
 });
 block.overrides.forEach(function(o){
  ...
  result.push({day:day,start:o.start||entry.start,end:o.end||entry.end,location:Object.prototype.hasOwnProperty.call(o,'location')?o.location:(entry.location||block.location||''),outcomeGoal:Object.prototype.hasOwnProperty.call(o,'outcomeGoal')?o.outcomeGoal:(block.outcomeGoal||''),sourceDate:o.sourceDate})
 });
 return result
}
function materializeDate(schedule,tasks,when,baseKey){
 ...
 schedule.forEach(function(block){if(!block.trackCompletion||block.category==='class')return;occurrenceEntries(block,when).forEach(function(entry){
  ...
  if(found){if(!found.rescheduled){var next={text:block.name||found.text,category:block.category,scheduledStart:start,scheduledEnd:end,plannedMinutes:duration,time:timeBand(duration),notes:notes,outcomeGoal:entry.outcomeGoal||'',scheduleId:block.id,sourceDate:entry.sourceDate,due:due,dueKey:key,setKey:baseKey};Object.keys(next).forEach(function(k){if(found[k]!==next[k]){found[k]=next[k];found.updatedAt=Date.now();changed=true}})}return}
  tasks.unshift({id:oid,text:block.name||'Scheduled action',pri:'should',time:timeBand(duration),due:due,dueKey:key,setKey:baseKey,done:false,doneAt:null,created:new Date(start).getTime(),updatedAt:Date.now(),order:new Date(start).getTime(),notes:notes,outcomeGoal:entry.outcomeGoal||'',category:block.category,source:'timetable',sourceDate:entry.sourceDate,scheduleId:block.id,occurrenceId:oid,scheduledStart:start,scheduledEnd:end,plannedMinutes:duration,outcome:null,startNudgedAt:null,missedAskedAt:null,rescheduled:false});changed=true
...
function installTimetableEditor(){
 ...
 var row=document.createElement('div');row.className='ab-editor-row';row.innerHTML='<div class="ab-field"><label class="f-label">Category</label><div class="ab-select-wrap"><select class="f-input ab-select" id="fCategory" aria-label="Schedule category"><option value="class">Class</option><option value="study">Study</option><option value="training">Training</option><option value="personal">Personal</option></select></div></div><div class="ab-field"><label class="f-label">Task creation</label><label class="ab-toggle" id="fTrackLabel" for="fTrack"><span class="ab-toggle-copy"><strong id="fTrackTitle">Add to to-do</strong><small id="fTrackHint">Fresh task on every selected day.</small></span><input class="ab-check" id="fTrack" type="checkbox"><span class="ab-switch" aria-hidden="true"><i></i></span></label></div>';host.insertAdjacentElement('afterend',row);
 var planning=document.createElement('details');planning.className='ab-planning';planning.id='fPlanning';planning.innerHTML='<summary>Planning details</summary><div class="ab-planning-body"><div class="ab-plan-field"><label class="f-label">Default target</label><input class="f-input" id="fOutcome" placeholder="Optional fallback"><div class="ab-plan-help" id="fOutcomeHelp">Leave blank if it changes. Set each session under This week’s targets.</div></div><div class="ab-term-row"><div class="ab-plan-field"><label class="f-label">Starts</label><input class="f-input" id="fStartDate" type="date"></div><div class="ab-plan-field"><label class="f-label">Ends</label><input class="f-input" id="fEndDate" type="date"></div></div><div class="ab-plan-help">Leave blank to repeat indefinitely. One-week changes are handled from the schedule card.</div></div>';row.insertAdjacentElement('afterend',planning);
 var cat=document.getElementById('fCategory'),track=document.getElementById('fTrack'),trackLabel=document.getElementById('fTrackLabel'),trackTitle=document.getElementById('fTrackTitle'),trackHint=document.getElementById('fTrackHint'),name=document.getElementById('fName'),desc=document.getElementById('fDesc'),overlay=document.getElementById('overlay'),outcome=document.getElementById('fOutcome'),outcomeHelp=document.getElementById('fOutcomeHelp'),startDate=document.getElementById('fStartDate'),endDate=document.getElementById('fEndDate');
...
function notionPayload(tasks){return parseList(tasks).filter(function(t){return t&&t.occurrenceId&&t.scheduledStart}).map(function(t){var duration=Math.max(1,Math.round((Date.parse(t.scheduledEnd||t.scheduledStart)-Date.parse(t.scheduledStart))/60000));return{notionPageId:t.notionPageId||undefined,occurrenceId:t.occurrenceId,scheduleId:t.scheduleId||undefined,action:t.text||'Scheduled action',category:cap(t.category)||'Personal',status:t.outcome==='skipped'?'Skipped':t.done?'Done':'Scheduled',scheduledStart:t.scheduledStart,scheduledEnd:t.scheduledEnd||null,plannedMinutes:duration,actualMinutes:t.actualMinutes||null,priority:cap(t.pri),source:t.source==='timetable'?'Schedule':'Manual',notes:[t.outcomeGoal?('Target: '+t.outcomeGoal):'',t.notes||''].filter(Boolean).join(' · ')}})}
...
function run(){if(!root.SyncEngine)return;normalizeSchedule();var tasks=parseList(root.SyncEngine.get('todo','tasks'));if(isTodoWidget){tasks=materialize()||tasks;checkPrompts(tasks);syncNotion(tasks)}applyCompletionMarks(tasks);lastDay=dateKey()}
function boot(){
