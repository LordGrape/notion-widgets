...
if(!action.includes('ADAPTIVE WEEKLY TARGETS v1')){
  ...
  action=replaceOnce(action,"Optional, e.g. brief cases 4–6","Optional fallback","Default target placeholder");
  ...
  action=replaceOnce(action,"class=\"ab-plan-help\">Leave blank if it changes.","class=\"ab-plan-help\" id=\"fOutcomeHelp\">Leave blank if it changes.","Default target helper id");
...
@media (max-width:400px){`,'Weekly target styles');
  ...
  timetable=replaceOnce(timetable,"+(item.description?'<div class=\"tip-detail\">'+esc(item.description)+'</div>':'');","+(item.outcomeGoal?'<div class=\"tip-detail\"><b>Target</b> · '+esc(item.outcomeGoal)+'</div>':'')+(item.description?'<div class=\"tip-detail\">'+esc(item.description)+'</div>':'');","Week tooltip target");
  timetable=replaceOnce(timetable,"+(c.description?'<span class=\"d-item\">'+esc(c.description)+'</span>':'')","+(c.outcomeGoal?'<span class=\"d-item\">Target <b>'+esc(c.outcomeGoal)+'</b></span>':'')\n      +(c.description?'<span class=\"d-item\">'+esc(c.description)+'</span>':'')","Day card target");
