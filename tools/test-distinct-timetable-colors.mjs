...
assert.equal(COLORS.length,12,'palette should stay compact at twelve colours');
...
assert.ok(html.includes("palette.indexOf(selColor)<0)palette.unshift(selColor)"),'older saved colours should remain editable');
assert.equal((html.match(/selColor=nextAvailableColor\(\)/g)||[]).length,3,'new, cleared, and post-save forms should rotate colours');
console.log('Distinct timetable colour tests passed');
