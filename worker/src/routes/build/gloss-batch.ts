...
import { BUILD_KEYS, kvGetJson, SAMPLE_GLOSSES, sampaToIpa } from '../../lib/french-core';
import type { Env } from '../../types';

function json(body: unknown, status = 200): Response { return new Response(JSON.stringify(body), { status, headers: { 'Content-Type': 'application/json' } }); }
const TOKEN_MAX = 3_000_000;
const TOKEN_WARN = 1_500_000;
...
export async function handleBuildGlossBatch(request: Request, env: Env): Promise<Response> {
  try {
    ...
    if (!missing.length) {
      ...
      return json({ status: 'done', batchIndex: 0, lemmasGlossed: 0, totalGlossed: Object.keys(cached.byLemmaPos).length, totalLemmas: lexique.length, cumulativeTokens: budget.cumulativeTokens });
    }
    ...
    const userPrompt = `For each French lemma below, return gloss and optional exampleHint. Input rows:\n${JSON.stringify(normalized)}\nReturn: {"glosses":[{"lemma":"...","pos":"...","gloss":"...","exampleHint":"..."}]}`;
    ...
    for (const g of out) {
      ...
      cached.byLemmaPos[key] = { lemma, pos, gloss, exampleHint: g.exampleHint ? String(g.exampleHint).trim() : undefined, source: 'llm' };
      saved++;
    }
    ...
    return json({ status: Object.keys(cached.byLemmaPos).length >= lexique.length ? 'done' : 'ok', batchIndex, lemmasGlossed: saved, totalGlossed: Object.keys(cached.byLemmaPos).length, totalLemmas: lexique.length, cumulativeTokens });
  } catch (err) {
