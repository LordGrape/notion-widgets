...
function buildSystemPrompt(body: LearnPlanRequest): string {
  ...
  return [
    ...
    "- Negative example (DO NOT emit): \"When was the regiment that became the (Unit) founded, and under what name?\" If tutorPrompt asks the same facts, a high verdict measures priming, not learning.",
    ...
    "- Negative example (DO NOT emit): \"The regiment was founded on 12 June 1885 as the 21st Essex Battalion in (Location). How do these details establish the regiment's identity and location?\" - this is a tautological closer where the answer is the restated premise.",
...
export async function handleLearnPlan(request: Request, env: Env): Promise<Response> {
  ...
  return makeSSEResponse(async (emit, signal) => {
    ...
    let groundingRejectedCount = 0;
    let parsedSegmentCount = 0;
    let secondParsedSegmentCount = 0;
    let secondGroundingRejectedCount = 0;
    let secondQualityRejectedCount = 0;
    ...
    try {
      for await (const chunk of streamGemini(
        ...
        for (const slice of completed) {
          ...
          if (!verifySegmentGrounding(seg, cardCorpus)) {
    ...
    if (emittedSegments.length >= minimumVerifiedSegments) {
    ...
    for (const segment of secondAttempt?.segments || []) {
      if (!segment || typeof segment !== "object") continue;
      secondParsedSegmentCount += 1;
      if (!verifySegmentGrounding(segment, cardCorpus)) {
        secondGroundingRejectedCount += 1;
        continue;
      }
      const quality = verifySegmentQuality(segment);
      if (!quality.ok) {
        secondQualityRejectedCount += 1;
        continue;
      }
      verifiedSecondBase.push(segment);
    }
